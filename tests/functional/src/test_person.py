...
async def test_person_films(person_index, make_get_request, redis_cache):
    ...
    film_ids: list[str] = [i.get("uuid") for i in expected.get("film_ids")]
    print(film_ids)
    body: dict = {
        "size": page_size,
    ...
    key: str = f"{response_body.get('total')}{page}{page_size}{body}"
    print(key)
